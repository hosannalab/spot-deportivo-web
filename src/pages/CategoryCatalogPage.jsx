...
function CategoryCatalogPage({ config }) {
  ...
  return (
    <SiteChrome activeNav={config.navKey}>
      <CatalogHero
        crumb={config.crumb}
        title={config.title}
        ...
        <div className="container">
          {error && (
            <div className="catalog-status catalog-status--error" data-reveal>
              No pudimos cargar el catalogo. Intenta de nuevo en unos minutos.
            </div>
          )}

          {!loading && !error && catalogItems.length === 0 && (
            <div className="empty-catalog" data-reveal>
              <strong>Sin productos por ahora</strong>
              Estamos actualizando este catalogo. Si buscas algo especifico,
